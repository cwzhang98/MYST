...
@dataclass
class W2vTransformerWithFeatFusionConfig(FairseqDataclass):
    ...
    mask_length: Optional[int] = field(default=10, metadata={"help": "length of mask span"})
    mask_selection: MASKING_DISTRIBUTION_CHOICES = field(
        default="static", metadata={"help": "how to choose mask length"}
    )
    mask_other: Optional[float] = field(
        default=0,
        metadata={
    ...
    no_mask_overlap: Optional[bool] = field(
        default=False, metadata={"help": "whether to allow masks to overlap"}
    )


@register_model("w2v_transformer_with_feat_fusion", dataclass=W2vTransformerWithFeatFusionConfig)
class W2vTransformerWithFeatFusionModel(FairseqEncoderDecoderModel):
    ...
    @classmethod
    def build_model(cls, cfg: W2vTransformerWithFeatFusionConfig, task):
    ...
    def build_encoder(cls, cfg, dict, embed_tokens):
        encoder = W2vTransformerWithFeatFusionEncoder(cfg, dict, embed_tokens)
        return encoder
...
class W2vTransformerWithFeatFusionEncoder(FairseqEncoder):
    def __init__(self, cfg: W2vTransformerWithFeatFusionConfig, dictionary, src_embedding):
    ...
    def build_acoustic_encoder(self, cfg: W2vTransformerWithFeatFusionConfig):
        ...
        self.reconstructon_projection = nn.Linear(
            self.shared_encoder_embed_dim,
            self.embed_tokens.weight.shape[0],
        )
        self.w2v_model.load_state_dict(ckpt["model"], strict=True)

    def build_shared_encoder(self, cfg: W2vTransformerWithFeatFusionConfig):
    ...
    def forward_shared_encoder(self, feats, padding_mask):
        for index, layer in enumerate(self.transformer_layers):
            x = layer(feats, padding_mask)
        return {
            "encoder_out": [x],  # T x B x C
    ...
    def compute_ctc_logits_and_lprobs(self, src_tokens, src_lengths):
        ...
        feature_lengths = (1 - padding_mask.int()).sum(dim=1)
        # cnn subsampling
        w2v_feature, feature_lengths = self.subsample_audio(w2v_features, feature_lengths)  # T x B x C
        padding_mask = lengths_to_padding_mask(feature_lengths)
        ctc_logits = self.ctc_projection(self.dropout_module(w2v_feature)).float()
        lprobs = nn.functional.log_softmax(ctc_logits, dim=-1)
        return ctc_logits, padding_mask, lprobs, w2v_feature

    def compute_reconstruction_lprobs(self, feats):
        logits = self.reconstructon_projection(self.dropout_module(feats)).float()
        lprobs = nn.functional.log_softmax(logits, dim=-1)
        return lprobs

    def position_embed(self, feats, padding_mask):
        if self.positional_embed is not None:
            positions = self.positional_embed(padding_mask).transpose(0, 1)  # T x B x C
            feats += positions
        return feats

    def mask_audio_feats(self, audio_feats, padding_mask, ctc_lprobs, blank_idx):
        masked_audio_feats = audio_feats.clone()
        T, B, _ = masked_audio_feats.size()
        # mask indices
        mask_indices = compute_mask_indices(
            ...
            self.cfg.mask_other,
            1,
            self.cfg.no_mask_overlap,
            0,
            False
        )
        mask_indices = torch.from_numpy(mask_indices).to(masked_audio_feats.device)
        ctc_results = (-ctc_lprobs.transpose(0, 1)).argmax(-1)
        no_blank_indices = ctc_results.ne(blank_idx)
        mask_indices = torch.logical_and(mask_indices, no_blank_indices)
        # mask token from embedding
        mask_id = torch.full((1,), blank_idx, device=masked_audio_feats.device).long()
        mask_token = self.embed_tokens(mask_id).squeeze(dim=0)
        masked_audio_feats[mask_indices.transpose(0, 1)] = mask_token
        if self.positional_embed is not None:
            masked_audio_feats = self.position_embed(masked_audio_feats, padding_mask)
        return masked_audio_feats, ctc_results, mask_indices
